{{--
    COPY of plugins/ecommerce::themes.customers.orders.view, overridden by the
    handmade-workflow plugin (View::prependNamespace) for one reason: the payment
    proof card asked the customer to upload proof of a payment the system had
    already recorded as completed — nonsense for wallet payments, which settle
    instantly. Handmade orders never hit this (they have no payment record at all),
    but a catalogue order paid from the wallet did.

    Only the condition on the line below differs. Re-sync if the ecommerce plugin
    changes this view.
--}}
...
@section('content')
    <div class="bb-customer-content-wrapper">
        ...
        @if ($order->isPaymentProofEnabled() && ! \Botble\HandmadeWorkflow\Services\QuoteService::isPaymentSettled($order))
