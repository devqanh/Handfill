{{--
    COPY of plugins/ecommerce::themes.includes.order-tracking-detail, overridden by
    handmade-workflow (View::prependNamespace) for one reason: the order status line
    showed the coarse core status while the timeline below showed the real production
    step, so the same page contradicted itself.

    Only that one line differs. Re-sync if the ecommerce plugin changes this view.
--}}
@if ($order)
                            ...
                            <div class="bb-order-info-section">
                                ...
                                <div class="bb-order-info-list">
                                    ...
                                    <div class="bb-order-info-item">
                                        ...
                                        <span class="value">
                                            @php
                                                // Show the real production step. The core status is a coarse
                                                // mapping (everything mid-flow reads "Processing"), which
                                                // contradicts the progress timeline further down this page.
                                                $hwStatus = $order->production_status;
                                                $hwValid = $hwStatus
                                                    && \Botble\HandmadeWorkflow\Enums\ProductionStatusEnum::isValid($hwStatus);
                                            @endphp

                                            {!! BaseHelper::clean(
                                                $hwValid
                                                    ? \Botble\HandmadeWorkflow\Enums\ProductionStatusEnum::of($hwStatus)->toHtml()
                                                    : $order->status->toHtml()
                                            ) !!}
                                        </span>
                                    </div>
