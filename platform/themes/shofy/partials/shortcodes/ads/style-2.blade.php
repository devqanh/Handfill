                    ...
                    <div class="tp-banner-item-2 p-relative z-index-1 grey-bg-2 mb-20 fix">
                        ...
                        @if($title = $ad->getMetaData('title', true))
                            <h3 class="tp-banner-title-2">
                                @if ($ad->url)
                                    <a href="{{ $ad->click_url }}" @if($ad->open_in_new_tab) target="_blank" rel="noopener noreferrer" @endif>
                                @endif
                        ...
                        @if($buttonLabel = $ad->getMetaData('button_label', true))
                            <div class="tp-banner-btn-2">
                                @if ($ad->url)
                                    <a href="{{ $ad->click_url }}" @if($ad->open_in_new_tab) target="_blank" rel="noopener noreferrer" @endif class="tp-btn tp-btn-border tp-btn-border-sm">
                                @endif
                                    {{ $buttonLabel }}
