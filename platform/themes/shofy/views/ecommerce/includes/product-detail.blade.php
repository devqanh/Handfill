    ...
    @if (EcommerceHelper::isReviewEnabled() && ($product->reviews_avg || theme_option('ecommerce_hide_rating_star_when_is_zero', 'no') === 'no') && (!EcommerceHelper::hideRatingWhenNoReviews() || $product->reviews_count > 0))
        <div class="tp-product-details-rating-wrapper d-flex align-items-center mb-10">
            <div class="tp-product-details-rating">
                <a href="{{ $product->url }}#product-review" data-bb-toggle="scroll-to-review" aria-label="{{ __('Read reviews') }}">
...
<x-core::form :url="route('public.cart.add-to-cart')" method="POST" class="product-form">
    ...
    @if (EcommerceHelper::isCartEnabled())
        ...
        <div @class(['tp-product-details-action-wrapper mt-3', 'tp-cart-disabled' => $isOutOfStock])>
            {{-- Control label, not a section heading: keep it out of the heading
                 outline (was an <h3> after an <h4>, breaking heading order). --}}
            <div class="tp-product-details-action-title">{{ __('Quantity') }}</div>
    ...
    {!! apply_filters('ecommerce_product_detail_after_cart_actions', null, $product) !!}
...
{!! apply_filters('ecommerce_product_detail_after_meta', null, $product) !!}
